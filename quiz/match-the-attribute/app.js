/*
  Match the Attribute - quiz logic.
  ...
  order, so every run opens on href and src and ramps into table headers and
  the page head.

  The shape is deliberately the same as the Understanding Tags quiz next door
  (same modes, same combo scoring, same ready and results screens), so a
  learner moving between the two never has to re-learn the UI. What differs is
  what an option is: here all four are the same tag written four ways, and the
  answer is a whole snippet rather than a tag name.

  Two things are stored in localStorage, separately from each other:
    - the set of questions ever answered correctly, which is what the dashboard
...
(function () {
  ...
  const ACTIVITY_ID = "match-the-attribute";
  const STATE_KEY = "attribute_quiz_state_v1";
  const BEST_KEY = "attribute_quiz_best_v1";
  ...
  */
  ...
  /*
    ...
    through every tier instead of ending while still on the easy ones. A
    little slower than the tag quiz's estimate on purpose: an option here is a
    whole snippet to compare against three near-identical ones, not a single
    tag name read at a glance.
  */
  const SECONDS_PER_QUESTION = 4;
  ...
  */
  ...
  function allQuestions() {
    const out = [];
    ATTRIBUTE_QUIZ_TIERS.forEach(function (tier) {
  ...
  function totalQuestionCount() {
    return ATTRIBUTE_QUIZ_TIERS.reduce(function (sum, tier) {
  ...
  */
  function buildQueue(tierId, seconds) {
    const tiers = tierId
      ? ATTRIBUTE_QUIZ_TIERS.filter(function (tier) {
          return tier.id === tierId;
        })
      : ATTRIBUTE_QUIZ_TIERS;
    ...
    /*
      ...
      link and image questions, so the learner never meets a form or a table
      in the mode they play most. Instead, take a slice off the front of each
  ...
  /*
    ...
    with the questions not yet mastered first, so a second pass is spent on
    the ones the learner still needs.
  */
  ...
  function buildSidebar() {
    ...
    const untimedCount = ATTRIBUTE_QUIZ_TIERS.length + 1;
    const untimed = makeGroup("Untimed", untimedCount, true);
    untimed.body.appendChild(
      makeModeItem(
        "Every question",
        "All " + totalQuestionCount() + " questions, easiest first",
        {
          modeId: "untimed-all",
          mode: "untimed",
        }
    ...
    ATTRIBUTE_QUIZ_TIERS.forEach(function (tier) {
      untimed.body.appendChild(
        makeModeItem(tier.name, tier.questions.length + " questions, no clock", {
  ...
  */
  ...
  function showQuestion() {
    ...
    // The tag sits in the panel header rather than the prompt, so the learner
    // always knows which element they are dressing without re-reading it.
    dom.promptTag.innerHTML = tagMarkup(question.tag);
  ...
  function renderOptions(question) {
    ...
    shuffleArray(question.options).forEach(function (snippet) {
      const button = document.createElement("button");
      button.type = "button";
      button.className = "option-card";
      // The snippet is the identity of the option as well as its label, so it
      // is what the click handler compares against the answer.
      button.dataset.snippet = snippet;
      button.innerHTML = '<code>' + escapeHtml(snippet) + "</code>";
  ...
  */
  ...
  function endRun(reason) {
    ...
    if (reason === "time") {
      ...
      dom.resultsTitle.textContent = "All questions answered";
    }
  ...
  function renderResultsList(history) {
    ...
    history.forEach(function (row) {
      ...
      // Snippets are long enough to need their own lines here, unlike the tag
      // names in the quiz next door, so a wrong answer is stacked above the
      // right one rather than run together in a sentence.
      const answerLine = row.correct
        ? '<div class="results-answer-line">Correct: <code class="answer-right">' +
          escapeHtml(row.question.answer) +
          "</code></div>"
        : '<div class="results-answer-line">You picked <code class="answer-wrong">' +
          escapeHtml(row.picked) +
          '</code></div><div class="results-answer-line">The answer is <code class="answer-right">' +
          escapeHtml(row.question.answer) +
          "</code></div>";
  ...
  /*
    ...
    learner has seen every question, and the progress bar has to keep meaning
    that.
  */
